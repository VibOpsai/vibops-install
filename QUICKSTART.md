...
VibOps Connect supports managing VMs across three hypervisors. Set the appropriate environment variables on the gateway to enable VM operations.

**VMware vSphere / ESXi:**

| Variable | Example | Description |
|---|---|---|
| `VSPHERE_HOST` | `vcenter.internal.com` | vCenter hostname or IP (no `https://`) |
...
| `VSPHERE_PORT` | `443` | vCenter API port (default: 443) |
...
| `PROXMOX_USER` | `root@pam` | Proxmox user (or `user@pve`) |
| `PROXMOX_TOKEN_ID` | `vibops-api` | API token name |
| `PROXMOX_TOKEN` | `a1b2c3d4-...` | API token value |

> Create a Proxmox API token: Datacenter → Permissions → API Tokens → Add. Assign `PVEVMAdmin` role.
...
> Create an XO token: Settings → Users → your user → Tokens → Create.

### Example: gateway with vSphere

Add these to the gateway's `.env` or pass them in `docker-compose.yml`:

```bash
# In the gateway section of docker-compose.yml (or gateway .env)
CONNECT_GATEWAY_ID=your-gateway-id
CONNECT_TOKEN=your-gateway-token
VSPHERE_HOST=vcenter.internal.com
VSPHERE_USERNAME=administrator@vsphere.local
VSPHERE_PASSWORD=your-password
VSPHERE_VERIFY_SSL=false
```

After restart, the gateway will discover all VMs on the vCenter and report them to VibOps. You can then manage VMs from the console:

```
"List all VMs on the vSphere cluster"
"Migrate VM web-server-01 to host esxi-02"
"Run a VM waste scan — find stopped and over-provisioned VMs"
"Check backup compliance — which VMs have no recent snapshot?"
```
